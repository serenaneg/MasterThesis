...
for k = 1:numel(transects)
   % df = transec{k};
    ...
    dens_grid = NaN(longestSize, numProfiles);
    ...
    for i = 1:numProfiles
        ...
        profile_dens = df.(profileFields{i}).density;
        ...
        dens_grid(1:length(profile_dens), i) = profile_dens;
    ...
    dens_grid = dens_grid(:, sort_lat);

    %calculate distance GRIDDED
    % Define a reference latitude
    ref_latitude = max(lat);  % Reference latitude (most on shore station)
    ...
    %x = x_min:dx:x_max;
    new_x = 0:1.5:max(dist);  % New X-axis coordinates
    new_y = 5:5:150;  % New Y-axis coordinates
    ...
    density2 = gsw_rho(sal_grid, temp_grid, press_grid);

    density_grid= density - 1000;
    density2_grid = density2 - 1000;
    
   % RegSigmatheta = interp2(sort(dist, 'ascend'), a, density_grid(b,:), origRegX, origRegY, 'linear');
    RegSigmatheta = interp2(sort(dist, 'ascend'), a, dens_grid(b,:), origRegX, origRegY, 'linear');
    %--------------------------------------------------------------------------
    ...
    ATC=cl_bblfcn(origRegX, origRegY, RegTheta, RegSigmatheta, avgsec, ...
        1, 0.01, 2, 1, [0 0.1 0.2 0.4  0.6 0.8 1 1.5 2], ...
        [0 3], [25.8 26.0 26.15 26.3 26.4]);
    ...
    ASC=cl_bblfcn(origRegX, origRegY, RegSalinity, RegSigmatheta, avgsec, ...
        2, 0.01, 2, 1, [0 0.1 0.2 0.4  0.6  0.8  1 1.5 2], ...
        [0 1], [25.8 26.0 26.15 26.3 26.4]);
