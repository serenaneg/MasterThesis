...
for i in time:
    f = chlor.sel(time=i)
    if f.count(axis=(0, 1)) >= 7182: #total point = 72*110 = 7980 => 80% = 6336, 50% = 3960
        print(i)
        valid_list.append(f)
...
#%%
month_list = ['Jan', '', 'Mar', '', 'May', '', 'Jul', '', 'Sept', '', 'Nov', '']
...
ax.text(x = 2003, y = 9.5, s = '(a)', size = 50, color = 'k')
ax.set_xticklabels(['2003','','2005','','2007','','2009','','2011', 
                    '','2013','','2015','','2017','','2019', ''], fontsize = 30)
ax.set_ylabel('Number of enhancements per year', fontsize=30)
ax.set_xlabel('Year', fontsize = 30)
ax.grid(linewidth=1, color='gray', linestyle='--', alpha = .5)
ax.set_facecolor("whitesmoke")
fig.suptitle("Number of shelf-break's enhancements per year", fontsize = 42, y = 1.0)
fig.tight_layout()
fig.savefig(plot_path + 'num_enhancement_year.png', bbox_inches='tight', dpi = 100)
#%%
...
ax.set_ylabel('Number of enhancements', fontsize=30)
ax.set_xlabel('Month', fontsize = 30)
ax.grid(linewidth=1, color='gray', linestyle='--', alpha = .5)
ax.text(x = 11, y = 22, s = '(b)', size = 40, color = 'k')
ax.set_facecolor("whitesmoke")
fig.suptitle("Number of shelf-break's enhancements per month", fontsize = 35, y = 1.0)
fig.tight_layout()
plt.show()
...
ax1.text(x = 270, y = 5, s = '(b)', size = 50, color = 'k')


ax1.set_xlabel('Day of the year', fontsize = 40)
...
ax2.set_xticklabels(['2003','2004', '2005','2006','2007','2008','2009','2010','2011', 
                    '2012','2013','2014','2015','2016','2017','2018','2019', '2020'])
ax2.xaxis.set_tick_params(labelsize = 27)
ax2.yaxis.set_tick_params(labelsize = 30)
ax2.set_facecolor("whitesmoke")
ax2.set_xlabel('Year', fontsize = 40)
ax2.grid(lw=.5, linestyle = '--')
ax2.text(x = 6400, y = 5, s = '(a)', size = 50, color = 'k')


ax2.set_ylabel('Chl-a concentration [mg/m$^3$]', fontsize = 40)

fig.suptitle("Enhanced Chlorophyll days", fontsize = 55, y = 1.)
fig.tight_layout()
fig.savefig(plot_path + "enhancement.png", bbox_inches='tight')  #%%
#PRINT DATE FOR ENHANCMENT WITH CHL > 6
#%%
# Open the file in write mode
with open(plot_path + 'ehnancements_90%.txt', 'w') as file:
    ...
    for i in range(0, len(enhance)):
        # if enhance.values[i] > 1 :
            ...
            file.write(str(index[i]) +',' + str(result) + '\n')
