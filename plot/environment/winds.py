...
for i, ax in enumerate(axes.flat):
    ...
    lines = ax.contour(lons, lats, -depth, levels = depths, colors='black', linewidths = .55)
    ax.clabel(lines, inline=3, fontsize=50, colors = 'black')
    lines2 = ax.contour(lons, lats, -depth, levels = [200], colors='black', linewidths = 3)
    # ax.clabel(lines2, inline=3, fontsize=50, colors = 'black')
...
cbar = plt.colorbar(scalar, format='%.2f', spacing='proportional', cax=cax,
                    shrink=0.9, orientation = 'vertical', location = "right", pad = 0.1)
cbar.set_label( label = "Wind Speed [m/s]", fontsize = 90, y = 0.5, labelpad = 40)
cbar.ax.tick_params(which='minor', size=60, width=1, color='k', direction='in')
cbar.ax.tick_params(which='major', size=60, width=1, color='k', direction='in', labelsize = 60)
# Adjust the position and size of the colorbar axis
cax.set_position([1.02, 0.1, 0.02, 0.8])

# set the title of the figure
fig.suptitle("Monthly Wind speed and direction (01-01-2003 / 31-12-2020)", fontsize = 100, y = 0.98)
