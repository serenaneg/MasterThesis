...
dx = np.diff(lons)[0] * 111121
dy = -np.diff(lats, axis = 0)[0] * 111121
...
for i in range(0, 6575):
    ...
    w = ((1/(rho_ocean*f)) * ((dtau_y[i,:,:]/dx) - (dtau_x[i,:,:]/dy)))
    w_ek.append(w)
    
    b = (tau_x_arr[i,:,:])/(rho_ocean*f) 
    ui.append(b)
...
ui_ds = ui_coords.to_dataset(name='ui')

ekman = xr.merge([Mx_ek_ds, My_ek_ds, w_ek_ds, ui_ds], compat='override')
